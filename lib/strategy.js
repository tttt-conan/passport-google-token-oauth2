...
function OAuth2Strategy(options, verify) {
	...
	var parsedUrl = url.parse(this._userProfileURL);
	if (parsedUrl.pathname.indexOf('/userinfo') == parsedUrl.pathname.length - '/userinfo'.length) {
...
OAuth2Strategy.prototype.authenticate = function(req, options) {
	...
	if (req.query && (req.query.code || req.query.access_token)) {
		function loaded(err, ok, state) {
			...
			var callback = function(err, accessToken, refreshToken, params) {
				if (err) {
			...
			if (!access_token) {
				self._oauth2.getOAuthAccessToken(code, params, callback);
			} else {
